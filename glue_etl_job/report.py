...
args = getResolvedOptions(
    sys.argv, [JOB_NAME, GLUE_DB_NAME, GLUE_TABLE_NAME, OUT_BUCKET]
)
...
dynamic_frame = glue_ctx.create_dynamic_frame_from_catalog(
...
with pandas.ExcelWriter(buffer, engine="xlsxwriter") as writer:
    ...
    by_generation.to_excel(
        writer, sheet_name="Report", startcol=start_col, index=False, header=True
    )
    ...
    header_format = workbook.add_format(
        {"bold": True, "font_color": "white", "bg_color": "#4F81BD", "align": "center"}
    )

    data_commons = {"align": "center", "valign": "vcenter"}
